...
<script>
	export default {
		...
		computed: {
			isShow() {
				const isShow = !this.isApi || this.object.show
					? { 'display': 'block' }
					: { 'display': 'none' }
				return isShow
			},
			cfBorder() {
				const cfBorder = (this.isApi ? this.object.showCancel : this.showCancel)
			...
			animation() {
				const animation = (this.isApi ? this.object.animate : this.animate) 
				 ? 'animation'
				 : ''
				return animation
			},
